...
while($chatbot = mysqli_fetch_assoc($chatbots)) {
    // Fetch each chatbot name and id
    $dir = $chatbot['name'];
    $id = $chatbot['id'];

    // Create a subdirectory with the chatbot's name
    mkdir("../export/{$dir}/data", 0777, true);

    // Create sub files of domain.yml and nlu.yml in required structure
    $domain = fopen("../export/{$dir}/domain.yml", 'w') or die("Unable to open domain file!");
    $nlu = fopen("../export/{$dir}/data/nlu.yml", 'w') or die("Unable to open nlu file!");
